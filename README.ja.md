...
- **Process Info**: 選択したプロセスのメトリクス、イメージとランタイム、開いているファイル、DLL、環境変数、ネットワークのアドレスとポートを 1 つのダイアログから調査できます。Scheduling タブでは CPU の優先度と、プロセッサグループが 1 つの環境での CPU アフィニティを確認・変更し、その場で適用したり直前の値に戻したりできます。
- **ネットワークの調査**: `MENU > Investigate > Network endpoints` からシステム全体の TCP/UDP のアドレス、ポート、所有プロセスを確認し、Process Info に移動できます。一覧は操作時に取得し、レコーディングには含めません。
...
- ハンドル数の推移と Process Info の Files を確認し、ファイルの閉じ忘れがないか調べる。各ファイルハンドルのキャッシュ設定、Write-through、同期・非同期対応、アクセス権も確認できます。
- プロセスが読み込んでいる DLL のパスやバージョンを調べる。
- **Investigate → Find file users** からファイル名やパスで使用中のプロセスを検索し、そのプロセスの Files タブで詳しく調べる。検索できなかった範囲も確認できます。
